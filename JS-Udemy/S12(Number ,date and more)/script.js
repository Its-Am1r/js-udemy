...
const formatMovementDate = function (date, local) {
  const now = new Date();
  const daysPassed = Math.round((now - date) / (1000 * 60 * 60 * 24));
...
const startLogoutTimer = function () {
  const tick = function () {
    ...
    //in each call print time reemaining time to ui
...
btnLogin.addEventListener("click", function (e) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    // localDate.textContent = `${day}/${month}/${year},${hour}:${min}`;
...
// //calculate redius of circle
// console.log(Math.PI * Number.parseFloat('10px') ** 2);

// //random dice roll
// console.log(Math.floor(Math.random()) * 6 + 1);

// //create random numbers between two given numbers
